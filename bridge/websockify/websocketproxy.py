    ...
    def do_proxy(self, target):
        ...
        while True:
            ...
            if target in outs:
                ...
                dat = tqueue.pop(0)
                '''
                The below addition is specifically for symetrix controller.
                It adds a "\r" at the end of the string. The symetrix controller
                does not work without that
                '''
                _decoded_dat = dat.decode('latin1').encode('unicode_escape').decode('ascii')
                # Making sure it only works with Symetrix
                if _decoded_dat.lower().startswith(('csq','pur')):
                    dat = dat.decode('latin1').encode('unicode_escape').decode('ascii') + '\r'
